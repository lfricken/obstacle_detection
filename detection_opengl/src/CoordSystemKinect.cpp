...
#include "../include/CoordSystemKinect.hpp"
...
Vec3f GetCartCoord(int x, int y, const uint16_t* pDepth)
{
    ...
    if(distance < 500)
        distance = 0;
    Vec2f azmuthPolar(GetAngle(x,y));//theta is CCW angle from +X, Phi is angle from (+Z)Y plane
    ...
    cartesian.x = radius*sinL(azmuthPolar.y);//strange
    cartesian.y = cartesian.x*tan(azmuthPolar.x);
    cartesian.z = radius*cosL(azmuthPolar.y);
    **/
    /** IT WASN'T SPHERICAL
    cartesian.x = radius*sinL(azmuthPolar.y)*cosL(azmuthPolar.x);
    cartesian.y = radius*sinL(azmuthPolar.y)*sinL(azmuthPolar.x);
    cartesian.z = radius*cosL(azmuthPolar.y);
    **/
    return cartesian;
}
